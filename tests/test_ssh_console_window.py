...
def test_ssh_console_applies_presets_and_cleans_output():
    app = _app()
    page = SshConsolePage(presets=[SshPreset("Pilot", "10.77.0.1", "analysis-user", password="analysis-pass")])
    try:
        app.processEvents()
        ...
        assert page.password_edit.text() == "analysis-pass"
...
def test_default_analysis_ssh_presets_include_pilot_link_and_routed_rov():
    ...
    assert [(preset.name, preset.host, preset.username, preset.password) for preset in presets[:3]] == [
        ("Triton Pi", "tritonpi.local", "triton", "triton"),
