...
<body>
  ...
  <script type="module">
    ...
    const distanceSignificantDigits = 4;

    function formatSignificantValue(value, significantDigits = distanceSignificantDigits) {{
      if (!Number.isFinite(value)) return '-';
      if (value === 0) return (0).toFixed(significantDigits - 1);
    ...
    function formatDistance(meters, unit = unitSelect.value) {{
      ...
      return `${{formatSignificantValue(value)}} ${{unit}}`;
    }}
