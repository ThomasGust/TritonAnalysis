...
DEFAULT_FAST_GEOMETRY_MODE = "flat_luma"
DEFAULT_FAST_DISTORTION_MODEL = "KplusBrown4WithTangential2"
LEGACY_GEOMETRY_MODE = "enhanced"
LEGACY_DISTORTION_MODEL = "Brown4WithTangential2"
...
class ComponentSummary:
    """Parsed quality summary for one RealityScan alignment component."""
...
def make_water_corrector(enabled: bool) -> object | None:
    """Construct the optional TritonPilot water-correction helper when requested."""
    if not enabled:
        return None
    candidates = [
        Path(os.environ.get("TRITON_PILOT_ROOT", "")).expanduser(),
        REPO_ROOT.parent / "TritonPilot",
    ]
    for candidate in candidates:
        if candidate and (candidate / "video" / "frame_correction.py").exists() and str(candidate) not in sys.path:
    ...
    try:
        ...
        raise RuntimeError(
            "Water correction requires a TritonPilot checkout. Set TRITON_PILOT_ROOT "
            "or leave --rectify-water disabled."
        ) from exc
...
def write_realityscan_xmp(
    ...
    *,
    ...
    position_xml = ""
    if include_rig:
        rig_guid = _xmp_guid(f"triton_analysis:{calibration.rig_id}:{calibration.path}")
        rig_instance = _xmp_guid(f"triton_analysis:{calibration.rig_id}:{pair.stem}:{pair.index}")
...
def _mesh_large_face_filter_payload(result: MeshLargeFaceFilterResult) -> dict:
    ...
    return {
        "schema": "triton_analysis.mesh_large_face_filter",
        "schema_version": 1,
...
def _metric_scale_payload(result: MetricScaleResult) -> dict:
    ...
    return {
        "schema": "triton_analysis.metric_scale",
        "schema_version": 1,
...
def _distortion_slug(distortion_model: str) -> str:
    """Return a compact variant-name suffix for a RealityScan distortion model."""
    return {
        "Division": "division",
        "Brown3": "brown3",
        "Brown4": "brown4",
        "Brown3WithTangential2": "brown3t2",
        "Brown4WithTangential2": "brown4",
        "KplusBrown3WithTangential2": "kplus_brown3t2",
        "KplusBrown4WithTangential2": "kplus",
    }.get(distortion_model, re.sub(r"[^a-z0-9]+", "_", distortion_model.lower()).strip("_") or "custom")


def _variant_name_for_geometry(geometry_mode: str, distortion_model: str) -> str:
    return f"{geometry_mode}_{_distortion_slug(distortion_model)}"
...
def build_variant_specs(args: argparse.Namespace) -> list[VariantSpec]:
    ...
    base_name = _variant_name_for_geometry(base_geometry_mode, args.distortion_model)
    base = VariantSpec(
        name=base_name,
    ...
    kplus = DEFAULT_FAST_DISTORTION_MODEL
    ...
    if base.name not in {variant.name for variant in variants}:
...
def write_alignment_command_file(
    ...
    commands = [
        "# Generated by triton_analysis.realityscan.underwater_pipeline",
...
def _parse_int(value: str) -> int:
    """Extract the first integer-like number from a report value."""
    match = re.search(r"\d+", value.replace(",", ""))
    return int(match.group(0)) if match else 0


def _parse_component_summaries(text: str, *, selected_image_count: int) -> tuple[ComponentSummary, ...]:
    """Parse per-component alignment details from a RealityScan overview report."""
    pattern = re.compile(
        r'<p\s+class="itemTitle">\s*Component:\s*(.*?)\s*</p>(.*?)(?=<p\s+class="itemTitle">\s*Component:|\Z)',
        flags=re.DOTALL | re.IGNORECASE,
    )
    components: list[ComponentSummary] = []
    denom = max(selected_image_count, 1)
    for match in pattern.finditer(text):
        name = _strip_report_text(match.group(1)) or f"Component {len(components)}"
        ...
        registered_images = int(registered_match.group(1)) if registered_match else _parse_int(registered)
        components.append(
            ComponentSummary(
                name=name,
                registered_images=registered_images,
                selected_image_count=selected_image_count,
                registered_ratio=registered_images / denom,
                points_count=_parse_int(props.get("Points' count", "")),
                total_projections=_parse_int(props.get("Total projections", "")),
                average_track_length=_parse_float(props.get("Average track length", "")),
                mean_reprojection_error_px=_parse_float(props.get("Mean reprojection error [pixels]", "")),
            )
        )
...
def parse_alignment_report(
    ...
    *,
    ...
    components = _parse_component_summaries(text, selected_image_count=selected_image_count)
    registered_counts = [component.registered_images for component in components]
    point_counts = [component.points_count for component in components]
    reprojection_errors = [component.mean_reprojection_error_px for component in components if component.mean_reprojection_error_px > 0]
...
def write_alignment_results(paths: OutputPaths, results: list[AlignmentResult]) -> None:
                    ...
                    "top_components": [
                        {
                            "rank": rank,
                            "name": component.name,
                            "registered_images": component.registered_images,
                            "registered_ratio": component.registered_ratio,
                            "points_count": component.points_count,
                            "total_projections": component.total_projections,
                            "average_track_length": component.average_track_length,
                            "mean_reprojection_error_px": component.mean_reprojection_error_px,
                        }
                        for rank, component in enumerate(ranked_components(result.components)[:12], start=1)
                    ],
                }
                for result in results
            ],
    ...
    write_alignment_component_details(paths, results)


def ranked_components(components: Iterable[ComponentSummary]) -> list[ComponentSummary]:
    """Return components ranked by likely inspection value."""
    return sorted(
        components,
        key=lambda component: (
            component.registered_images,
            component.points_count,
            component.total_projections,
            -component.mean_reprojection_error_px,
        ),
        reverse=True,
    )


def write_alignment_component_details(paths: OutputPaths, results: list[AlignmentResult]) -> None:
    """Write a flat table of all parsed components across alignment variants."""
    csv_path = paths.reports / "alignment_components.csv"
    json_path = paths.reports / "alignment_components.json"
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        ...
        for result in results:
            for rank, component in enumerate(ranked_components(result.components), start=1):
                row = {
                    "variant": result.name,
                    "rank": rank,
                    "component": component.name,
                    "registered_images": component.registered_images,
                    "selected_image_count": component.selected_image_count,
                    "registered_ratio": component.registered_ratio,
                    "points_count": component.points_count,
                    "total_projections": component.total_projections,
                    "average_track_length": component.average_track_length,
                ...
                writer.writerow(
                    [
                        ...
                        row["rank"],
                        row["component"],
                        row["registered_images"],
                        row["selected_image_count"],
                        f"{float(row['registered_ratio']):.6f}",
                        row["points_count"],
                        row["total_projections"],
                        f"{float(row['average_track_length']):.6f}",
                        f"{float(row['mean_reprojection_error_px']):.6f}",
                        row["report"],
                        row["project"],
                    ]
                )
...
def apply_legacy_enhanced_default(args: argparse.Namespace) -> None:
    """Restore the previous fast default when the compatibility switch is set."""
    if getattr(args, "legacy_enhanced_default", False):
        args.base_geometry_mode = LEGACY_GEOMETRY_MODE
        args.distortion_model = LEGACY_DISTORTION_MODEL
...
def write_realityscan_command_file(
    ...
    *,
    ...
    commands.append(f"-addFolder {_path_for_cli(frames_dir)}")
    if getattr(args, "texture_layers", False):
        commands.extend(
            [
                '-set "ImageLayerForColoring=texture01"',
                '-set "ImageLayerForTexturing=texture01"',
            ]
        )
    commands.append("-selectAllImages")
...
def _component_slug(component: ComponentSummary, *, rank: int) -> str:
    name = re.sub(r"[^a-zA-Z0-9]+", "_", component.name).strip("_").lower() or f"component_{rank:02d}"
    return f"rank_{rank:02d}_{name}_{component.registered_images:03d}imgs"


def top_diagnostic_components(
    result: AlignmentResult,
    *,
    max_count: int,
    min_images: int,
) -> list[ComponentSummary]:
    """Return the top components worth exporting for visual diagnostics."""
    return [
        component
        for component in ranked_components(result.components)
        if component.registered_images >= min_images
    ][:max_count]


def write_component_diagnostics_command_file(
    paths: OutputPaths,
    result: AlignmentResult,
    args: argparse.Namespace,
) -> tuple[Path, list[dict[str, object]]]:
    """Write an RSCMD file that exports top alignment components for inspection."""
    components = top_diagnostic_components(
        result,
        max_count=args.component_diagnostic_count,
        min_images=args.component_diagnostic_min_images,
    )
    export_dir = paths.root / "component_diagnostics"
    export_dir.mkdir(parents=True, exist_ok=True)
    rscmd = paths.root / "component_diagnostics.rscmd"
    commands = [
        "# Generated by triton_analysis.realityscan.underwater_pipeline",
        f"# Component diagnostics from {result.name}",
        f"-load {_path_for_cli(result.project)} deleteAutosave",
        "-setMinComponentSize 1",
    ]
    exports: list[dict[str, object]] = []
    for rank, component in enumerate(components, start=1):
        slug = _component_slug(component, rank=rank)
        component_file = export_dir / f"{slug}.rsalign"
        sparse_cloud = export_dir / f"{slug}.xyz"
        component_commands = [
            f'-selectComponent "{component.name}"',
            f"-exportSelectedComponentFile {_path_for_cli(component_file)}",
        ]
        ...
        if getattr(args, "component_diagnostic_sparse_clouds", False):
            component_commands.append(f"-exportSparsePointCloud {_path_for_cli(sparse_cloud)}")
            export["sparse_cloud"] = str(sparse_cloud)
        commands.extend(component_commands)
        exports.append(export)
    commands.append("-quit")
    rscmd.write_text("\n".join(commands) + "\n", encoding="utf-8")
    return rscmd, exports
...
def run_component_diagnostics(
    ...
    """Export top alignment components without making dense meshes."""
    components = top_diagnostic_components(
        result,
        max_count=args.component_diagnostic_count,
        min_images=args.component_diagnostic_min_images,
    ...
    if not components:
        print(
            "Component diagnostics skipped: no components reached "
            f"{args.component_diagnostic_min_images} registered images."
        )
        return 0
    rscmd, exports = write_component_diagnostics_command_file(paths, result, args)
    progress = paths.logs / "component_diagnostics_progress.txt"
    stdout_log = paths.logs / "component_diagnostics_stdout.txt"
    crash_reports = paths.crash_reports / "component_diagnostics"
    crash_reports.mkdir(parents=True, exist_ok=True)
    expected = [Path(item["component_file"]) for item in exports]
    print(
        f"Exporting {len(exports)} component diagnostic(s) from {result.name} "
        f"to {paths.root / 'component_diagnostics'}..."
    )
    ...
    summary = {
        "alignment": result.name,
        "project": str(result.project),
        "report": str(result.report),
        "exit_code": code,
        "exports": exports,
    }
    summary_path = paths.reports / "component_diagnostics.json"
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(f"Component diagnostics exported: {paths.root / 'component_diagnostics'}")
    print(f"Component diagnostics summary: {summary_path}")
    return code
    ...
    manifest = {
        ...
        if best_alignment is None
            ...
            "top_components": [
                ...
                for rank, component in enumerate(ranked_components(best_alignment.components)[:12], start=1)
            ],
        },
...
def run_alignment_tournament(
    ...
    for spec in variant_specs:
        if args.alignment_tournament == "off" and not args.alignment_only:
            continue
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--alignment-only", action="store_true", help="Run alignment and stop before meshing/texturing. With tournament off, this tests the single fast variant.")
    ...
    parser.add_argument("--export-component-diagnostics", action=argparse.BooleanOptionalAction, default=False, help="Export top alignment components as .rsalign files for fragment inspection.")
    parser.add_argument("--component-diagnostic-count", type=int, default=8, help="Maximum number of ranked components to export when component diagnostics are enabled.")
    parser.add_argument("--component-diagnostic-min-images", type=int, default=3, help="Minimum registered images required for a component diagnostic export.")
    parser.add_argument("--component-diagnostic-sparse-clouds", action="store_true", help="Also try RealityScan sparse point cloud exports for diagnostic components. Off by default because .rsalign component export is more reliable.")
    parser.add_argument(
    ...
    parser.add_argument("--distortion-model", default=DEFAULT_FAST_DISTORTION_MODEL, help="RealityScan lens distortion model.")
    ...
    parser.add_argument("--min-good-component-ratio", type=_positive_float, default=DEFAULT_MIN_GOOD_COMPONENT_RATIO, help="Warn when the best alignment's largest component is below this fraction of selected frames.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.component_diagnostic_count < 1:
        parser.error("--component-diagnostic-count must be at least 1")
    if args.component_diagnostic_min_images < 1:
        parser.error("--component-diagnostic-min-images must be at least 1")
    source_type = "video"
    stereo_session: StereoSessionData | None = None
    stereo_calibration: StereoCalibration | None = None
    ...
    if len(variant_specs) == 1:
        ...
        print(
            f"Writing selected frames: {spec.name} "
            f"(geometry={spec.geometry_mode}, distortion={spec.distortion_model or args.distortion_model})..."
        )
    else:
    ...
    for spec in variant_specs:
        if args.alignment_tournament != "off" or args.alignment_only:
    ...
    if args.prepare_only:
        ...
        if overview_template and (args.alignment_tournament != "off" or args.alignment_only):
            print(f"RealityScan report template: {overview_template}")
        elif args.alignment_tournament != "off" or args.alignment_only:
    ...
    if args.alignment_tournament != "off" or args.alignment_only:
        if overview_template is None:
            raise RuntimeError("RealityScan Overview.html report template was not found; cannot score alignment.")
        ...
        if best_alignment.largest_component_ratio < args.min_good_component_ratio:
            ...
            print(message)
        if args.export_component_diagnostics:
            diagnostic_code = run_component_diagnostics(paths, best_alignment, realityscan_exe, args)
            if diagnostic_code != 0:
                print(f"Warning: component diagnostics exited with code {diagnostic_code}.")
        if best_alignment.largest_component_ratio < args.min_good_component_ratio:
    ...
    if paths.model.exists():
        ...
        final_report = paths.reports / "final_overview.html"
        if final_report.exists() and not alignment_results:
            final_alignment = parse_alignment_report(
                final_report,
                name=best_spec.name,
                selected_image_count=len(variant_frame_paths[best_spec.name]),
                project=paths.project,
            )
            if final_alignment.components:
                alignment_results = [final_alignment]
                best_alignment = final_alignment
                write_alignment_results(paths, alignment_results)
                print(f"Component summary: {paths.reports / 'alignment_components.csv'}")
                write_manifest(
                    paths,
                    input_path,
                    info,
                ...
                if args.export_component_diagnostics:
                    diagnostic_code = run_component_diagnostics(paths, final_alignment, realityscan_exe, args)
                    if diagnostic_code != 0:
                        print(f"Warning: component diagnostics exited with code {diagnostic_code}.")
